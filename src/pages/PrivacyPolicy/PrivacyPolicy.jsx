    ...
    <div className="min-h-screen bg-[#1C1713] text-white">
      ...
      <section className="relative py-20 px-6 md:px-12 lg:px-24 border-b border-[#2c241c] overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-6xl font-normal leading-tight font-serif tracking-wide mb-6 uppercase">
          ...
          {sections.map((section, index) => (
            <div key={index} className="bg-[#15110F] p-8 md:p-10 border border-[#2c241c] rounded-none hover:border-[#c8956c]/50 transition-colors duration-300">
              <div className="flex items-center gap-4 mb-6">
                ...
                <h3 className="text-3xl md:text-4xl font-serif font-normal text-white uppercase tracking-wide m-0">
                  {section.title.split(' ').length > 1 ? section.title.split(' ').slice(0, -1).join(' ') + ' ' : ''}
                  <span className="text-[#c8956c]">{section.title.split(' ').slice(-1)}</span>
                </h3>
              </div>
